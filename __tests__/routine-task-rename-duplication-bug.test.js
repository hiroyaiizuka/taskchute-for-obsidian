...
describe("ルーチンタスクのリネームと複製によるバグ", () => {
  ...
  beforeEach(() => {
    ...
    global.localStorage = {
      ...
      key: jest.fn(
        (index) => Object.keys(global.localStorage.data)[index] || null,
      ),
      get length() {
  ...
  test("実行ログのタスク名が変更されないことを確認", async () => {
    ...
    taskChuteView.app.vault.getAbstractFileByPath = jest
      .fn()
      .mockResolvedValue(true)
    taskChuteView.app.vault.adapter.list = jest.fn().mockResolvedValue({
      files: ["TaskChute/Log/2024-01-tasks.json"],
    })
    taskChuteView.app.vault.adapter.read = jest
      .fn()
      .mockResolvedValue(JSON.stringify(monthlyLog))
    ...
    const logContent = await taskChuteView.app.vault.adapter.read(
      "TaskChute/Log/2024-01-tasks.json",
    )
    const log = JSON.parse(logContent)

    expect(log.taskExecutions["2024-01-15"][0].taskName).toBe("古い名前")
    expect(log.taskExecutions["2024-01-15"][0].taskId).toBe(
      "TaskChute/Task/古い名前.md",
    )
  })

  test("stopInstance時に複製情報は削除されない（修正後の正しい動作）", async () => {
    // 複製情報を設定
    ...
    taskChuteView.getTaskRecordDateString = jest
      .fn()
    ...
    // 修正後：stopInstance時は複製情報が保持されることを確認
    const remaining = JSON.parse(
      localStorage.getItem("taskchute-duplicated-instances-2024-01-15") || "[]",
    )
    expect(remaining).toHaveLength(2) // 修正前：削除されて1件、修正後：保持されて2件
    expect(remaining.find((r) => r.instanceId === "instance-1")).toBeDefined()
    expect(remaining.find((r) => r.instanceId === "instance-2")).toBeDefined()
  })
})
