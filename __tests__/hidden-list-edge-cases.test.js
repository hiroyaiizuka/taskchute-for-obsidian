...
describe('非表示リストのエッジケース', () => {
  ...
  describe('複製されたタスクの処理', () => {
    test('複製されたタスクの削除は元のタスクに影響しない', async () => {
      ...
      const hiddenInst2 = savedRoutines.find(r => r.instanceId === 'instance-2')
  ...
  describe('削除保護の動作確認', () => {
    ...
    test('完了済みタスクの各種状態での削除保護', async () => {
      ...
      for (const { state, expected } of states) {
        ...
        if (expected) {
          // 削除が拒否される
          expect(Notice).toHaveBeenCalledWith('完了済みのタスクは削除できません。')
          expect(view.taskInstances).toHaveLength(1)
        } else {
          // 削除が実行される
          expect(Notice).not.toHaveBeenCalledWith('完了済みのタスクは削除できません。')
          expect(view.taskInstances).toHaveLength(0)
        }
      }
    })
