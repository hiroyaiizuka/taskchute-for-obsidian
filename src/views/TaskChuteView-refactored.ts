...
async function shouldShowNonRoutineTask(this: any, file: any, metadata: any, dateString: string): Promise<boolean> {
  ...
  // Check if task has a target_date that matches today
  if (metadata?.target_date && metadata.target_date === dateString) {
    return true;
  }

  // Check file creation date - non-routine tasks are shown only on creation date
  try {
